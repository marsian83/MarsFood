...
function calcFilter(hexColor) {
  const rgb = hexToRgb(hex);
  if (!rgb || rgb.length !== 3) {
    console.log("Error : Invalid hexColor >" + hex + "<");
    return;
  }
  ...
  return result.loss < 5 ? result.filter : getFilter(hexColor);
}

function getFilter(hexColor) {
  hex = hexColor.trim();
  if (hex in filterCache) {
    return filterCache[hex];
  } else {
    filterCache[hex] = calcFilter(hex);
    return filterCache[hex];
  }
}
...
navbarItems.forEach((element) => {
  ...
  element.addEventListener("mouseout", () => {
    ...
    element.querySelector(".navbar-item img").style = getFilter(colorTextPrimary);
  });
});
